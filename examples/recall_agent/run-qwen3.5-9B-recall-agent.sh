...
HF_CKPT="${HF_CKPT:-/opt/users/models/Qwen3.5-9B}"
...
WANDB_GROUP="${WANDB_GROUP:-recall_agent_qwen3.5_9b}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-qwen3.5-9B-recall-agent-grpo}"
...
if [[ ! -d "${REF_LOAD}" ]]; then
  ...
  source "${ROOT_DIR}/scripts/models/qwen3.5-9B.sh"
